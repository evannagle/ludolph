...
**Flow:**
1. Validates `lu-plugin.toml` in current directory
2. Checks version format and required fields
3. Generates PR body with plugin metadata
4. Opens browser to create PR against `ludolph-community/plugin-registry`
...
**CLAUDE.md:** Use existing template from `examples/plugins/lu-example/CLAUDE.md`
...
3. Write scaffold files with template interpolation
4. Print success message with next steps

**plugin_publish():**
1. Read `lu-plugin.toml` from current directory
2. Validate required fields (name, version, description, repository)
3. Generate PR body markdown
4. Open browser to `github.com/ludolph-community/plugin-registry/compare`
...
```bash
# In a plugin directory with repository configured
lu plugin publish
# Should open browser or print PR URL
```
...
4. `lu plugin publish` successfully generates PR URL
5. `lu plugin search` returns results from registry
