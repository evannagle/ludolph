...
impl Memory {
    ...
    /// Count messages for a user that haven't been persisted to vault.
    #[allow(clippy::cast_sign_loss, clippy::cast_possible_truncation)]
    pub fn count_unpersisted(&self, user_id: i64) -> Result<usize> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| anyhow::anyhow!("Lock error: {e}"))?;
        ...
        let count: i64 = conn.query_row(
            "SELECT COUNT(*) FROM messages WHERE user_id = ?1 AND persisted = 0",
            params![user_id],
            |row| row.get(0),
        )?;

        // Safe: COUNT(*) is always non-negative and small
        Ok(count as usize)
    }

    /// Check if messages should be persisted to long-term storage.
    pub fn should_persist(&self, user_id: i64) -> Result<bool> {
        let count = self.count_unpersisted(user_id)?;
        Ok(count >= self.persist_threshold)
    }

    /// Get messages that need to be persisted to vault.
    ///
    /// Returns the oldest unpersisted messages (those beyond the window).
    pub fn get_messages_to_persist(&self, user_id: i64) -> Result<Vec<ConversationMessage>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| anyhow::anyhow!("Lock error: {e}"))?;
        ...
        let mut stmt = conn.prepare(
            "SELECT role, content, timestamp FROM messages
             WHERE user_id = ?1 AND persisted = 0
        ...
        // Only return messages beyond the window (keep recent ones local)
        if messages.len() > self.window_size {
            Ok(messages[..messages.len() - self.window_size].to_vec())
        } else {
            Ok(vec![])
        }
    }

    /// Mark messages as persisted to vault.
    ///
    /// Call this after successfully saving to vault via MCP.
    pub fn mark_persisted(&self, user_id: i64, up_to_timestamp: &DateTime<Utc>) -> Result<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| anyhow::anyhow!("Lock error: {e}"))?;
        ...
        conn.execute(
            "UPDATE messages SET persisted = 1 WHERE user_id = ?1 AND timestamp <= ?2",
            params![user_id, up_to_timestamp.to_rfc3339()],
        )?;

        Ok(())
    }

    /// Delete old persisted messages to free space.
    ///
    /// Keeps only unpersisted messages and recent window.
    pub fn cleanup(&self, user_id: i64) -> Result<usize> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| anyhow::anyhow!("Lock error: {e}"))?;

        // Delete persisted messages older than the window
        let deleted = conn.execute(
            "DELETE FROM messages
             WHERE user_id = ?1 AND persisted = 1
             AND id NOT IN (
            ...
            params![user_id, self.window_size],
        )?;
...
mod tests {
    ...
    #[test]
    fn persist_threshold_triggers() {
        let (memory, _dir) = test_memory();
        let user_id = 12345;

        // Add messages below threshold (8)
        for i in 0..7 {
            memory
                .add_message(user_id, "user", &format!("Message {i}"))
                .unwrap();
        }
        assert!(!memory.should_persist(user_id).unwrap());

        // Add one more to hit threshold
        memory.add_message(user_id, "user", "Message 7").unwrap();
        assert!(memory.should_persist(user_id).unwrap());
    }

    #[test]
    fn get_messages_to_persist_excludes_window() {
        let (memory, _dir) = test_memory();
        let user_id = 12345;

        // Add 8 messages with window_size=4
        for i in 0..8 {
            memory
                .add_message(user_id, "user", &format!("Message {i}"))
                .unwrap();
        ...
        let to_persist = memory.get_messages_to_persist(user_id).unwrap();
        // Should get 4 messages (8 - 4 window)
        assert_eq!(to_persist.len(), 4);
        assert_eq!(to_persist[0].content, "Message 0");
        assert_eq!(to_persist[3].content, "Message 3");
    }
